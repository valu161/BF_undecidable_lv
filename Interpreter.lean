...
/-Coercion from `Char` to `Nat`, to input `Data` symbolically. Coe is a already defined structure
and we now fill it with two instances, which tell us how to coerce `Char` to `Nat` and `List Char
` to `List Nat`.-/
...
def step (b : BrainState) : BrainState :=
  ...
  if h: b.progPos < b.prog.length then
    match b.prog.get ⟨b.progPos, h⟩ with
      ...
      | '+' => {b with
        progPos := b.progPos + 1,
        mem := b.mem.modify b.memPos (fun v => (v+1) % cellSize)}
      | '-' => {b with
        progPos := b.progPos + 1,
        mem := b.mem.modify b.memPos (fun v => (v-1) % cellSize)}
      | '.' => {b with
        progPos := b.progPos + 1,
        output := b.output ++ [b.mem.getD b.memPos 0]} -- getD sagt gib mir den index mempos
      -- von der liste, wenns den nicht gibt, gib mir 0
      | ',' => {b with
        progPos := b.progPos + 1,
        mem := List.modify b.mem b.memPos (fun _ => b.input.getD 0 0), --liest das erste element,
        --falls es das gibt, sonst 0
        input := b.input.drop 1} -- drop 1 entfernt das erste element der liste
      | '[' =>
        if h: b.mem.getD b.memPos 1 = 0 then
        match matchingClose b.prog b.progPos with
        |some x => {b with progPos:= x+1}
        |none => b
        else {b with progPos:= b.progPos +1}
      | ']' =>
        if h: b.mem.getD b.memPos 1 = 0 then
        {b with
        progPos := b.progPos +1}
        else
        match matchingOpen b.prog b.progPos with
        |some x => {b with progPos:= x+1}
        |none => b
...
def execute (p a : Data) (n : ℕ) : BrainState :=
  step^[n] { prog := p, input := a } --führt n mal step aus und damit verändert sich
                                      --der brainstate n mal

/- Useful for demonstration purposes macht aus dem output bei brainstate einen string -/
...
/-- the property that when running `p` on `a`, it will have halted after `n` steps (einfach
nur gleichheit als proposition). Der Type davon ist prop -/
